...
提醒时间使用北京时间，格式为 `YYYY-MM-DD HH:MM:SS`。控制台中暴涨提醒显示为绿色，暴跌提醒显示为红色，交易对以亮黄色突出；`alerts.beep` 开启时，macOS 使用系统 `Glass` 音效，其他平台使用终端响铃。文本明确显示价格涨跌百分比，不显示原始 ATR 数值和 24 小时成交额。JSONL 和 Webhook 记录仍保留完整结构化字段，并包含值为 `green` 或 `red` 的 `color` 字段。
